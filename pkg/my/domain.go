...
func Domains(client *node.Client, nickname string) ([]string, error) {
	...
	rawNames, err := node.DatastoreEntry(client, dns.GetDNSAddress(), ownerKey)
	if err != nil {
		return nil, fmt.Errorf("reading entry '%s' at '%s': %w", dns.GetDNSAddress(), ownedPrefix+wallet.Address, err)
	}
